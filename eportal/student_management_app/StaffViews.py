    ...
    if request.method != "POST":
        ...
        try:
            leave_report = LeaveReportStaff(
                staff_id=staff_obj, leave_date=leave_date, leave_message=leave_message, leave_status=0)
            leave_report.save()
...
def save_attendance_data(request):
    ...
    try:
        ...
        attendance = Attendance(
            subject_id=subject_model, attendance_date=attendance_date, session_year_id=session_year_model)
        attendance.save()
    ...
    if request.method != "POST":
        ...
        try:
            ...
            if check_exist:
                ...
                result.subject_third_test = third_test
                ...
                result = StudentResult(student_id=student_obj, subject_id=subject_obj,
                                       subject_exam_marks=exam_marks, subject_assignment_marks=assignment_marks, subject_first_test=first_test,
                                       subject_second_test=second_test, subject_third_test=third_test)
                result.save()
